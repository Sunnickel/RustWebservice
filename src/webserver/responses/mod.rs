...
//!
//! This module provides the [`HTTPResponse`] type, a high-level helper for
//! building valid HTTP/1.1 responses.  It delegates header manipulation to the
...
impl HTTPResponse {
    ...
    ///
    /// No headers (except the mandatory ones added later by `to_bytes`) and no
    /// body are set.
    ///
    /// # Example
    ///
...
impl HTTPResponse {
    ...
    pub fn set_json(&mut self) {
        use crate::webserver::http_packet::header::content_types::{
            ContentType, application::ApplicationSubType,
        };
